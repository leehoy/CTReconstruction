...
@cuda.jit('float32(int32,int32,int32,float32,float32,float32,float32,float32)', device=True)
def weight_calculator1(ind, s_index, e_index, coord1, coord2, dd, plane1, plane2):
    eps = 1e-5
    if s_index == e_index:
        weight = 1.0
    elif ind == s_index:
        weight = (max(coord1, coord2) - plane2) / abs(coord1 - coord2)
    elif ind == e_index:
        weight = (plane1 - min(coord1, coord2)) / abs(coord1 - coord2)
    else:
        weight = abs(dd) / abs(coord1 - coord2)
...
@cuda.jit('float32(int32,int32,int32,float32,float32,float32,float32,float32)', device=True)
def weight_calculator2(ind, s_index, e_index, coord1, coord2, dd, plane1, plane2):
    eps = 1e-5
    if s_index == e_index:
        weight = 1.0
    elif ind == s_index:
        weight = (plane2 - min(coord1, coord2)) / abs(coord1 - coord2)
    elif ind == e_index:
        weight = (max(coord1, coord2) - plane1) / abs(coord1 - coord2)
    else:
        weight = abs(dd) / abs(coord1 - coord2)
...
def distance_project_on_y(Dest, Src, slope_x1, slope_x2, slope_z1, slope_z2, intercept_x1, intercept_x2, intercept_z1,
    ...
    weight1 = 0.0
    weight2 = 0.0
    iy = int(floor((tid * 1.0) / (N * 1.0)))
    pix_num = int(tid - (N * iy))
    ...
    s_index_x = min(index_x1, index_x2)
    e_index_x = max(index_x1, index_x2)
    s_index_z = min(index_z1, index_z2)
    e_index_z = max(index_z1, index_z2)

    if tid < N * ny:
        for k in range(s_index_x, e_index_x + 1):
            if k >= 0 and k <= nx - 1:
                weight1 = weight_calculator2(k, s_index_x, e_index_x, coord_x1, coord_x2, dx, Xplane[k], Xplane[k + 1])
                for l in range(s_index_z, e_index_z + 1):
                    if l >= 0 and l <= nz - 1:
                        weight2 = weight_calculator1(l, s_index_z, e_index_z, coord_z1, coord_z2, dz, Zplane[l],
                                                     Zplane[l + 1])
                        cuda.atomic.add(Dest, pix_num + angle_ind * N,
                                        Src[(l * nx * ny) + iy * nx + k] * weight1 * weight2 * intersection_length[
                                            pix_num])
...
def distance_project_on_x(Dest, Src, slope_y1, slope_y2, slope_z1, slope_z2, intercept_y1, intercept_y2, intercept_z1,
    ...
    weight1 = 0.0
    weight2 = 0.0
    ix = int(floor((tid * 1.0) / (N * 1.0)))
    pix_num = int(tid - (N * ix))
    ...
    s_index_y = min(index_y1, index_y2)
    e_index_y = max(index_y1, index_y2)
    s_index_z = min(index_z1, index_z2)
    e_index_z = max(index_z1, index_z2)

    if tid < N * nx:
        for k in range(s_index_y, e_index_y + 1):
            if k >= 0 and k <= ny - 1:
                weight1 = weight_calculator1(k, s_index_y, e_index_y, coord_y1, coord_y2, dy, Yplane[k], Yplane[k + 1])
                for l in range(s_index_z, e_index_z + 1):
                    if l >= 0 and l <= nz - 1:
                        weight2 = weight_calculator1(l, s_index_z, e_index_z, coord_z1, coord_z2, dz, Zplane[l],
                                                     Zplane[l + 1])
                        # cuda.atomic.add(Dest, pix_num * N * angle_ind,
                        #                 Src[(l * nx * ny) + k * nx + ix] * weight1 * weight2)
                        cuda.atomic.add(Dest, pix_num + N * angle_ind,
                                        Src[(l * nx * ny) + k * nx + ix] * weight1 * weight2 * intersection_length[
                                            pix_num])
...
def distance_project_on_z(Dest, Src, slope_x1, slope_x2, slope_y1, slope_y2, intercept_x1, intercept_x2, intercept_y1,
    ...
    weight1 = 0.0
    weight2 = 0.0
    iz = int(floor((tid * 1.0) / (N * 1.0)))
    pix_num = int(tid - (N * iz))
    ...
    if tid < N * nz:
        for k in range(s_index_x, e_index_x + 1):
            if k >= 0 and k <= nx - 1:
                weight1 = weight_calculator2(k, s_index_x, e_index_x, coord_x1, coord_x2, dx, Xplane[k], Xplane[k + 1])
                for l in range(s_index_y, e_index_y + 1):
                    if l >= 0 and l <= ny - 1:
                        weight2 = weight_calculator1(l, s_index_y, e_index_y, coord_y1, coord_y2, dy, Yplane[l],
                                                     Yplane[l + 1])
                        cuda.atomic.add(Dest, pix_num + angle_ind * N,
                                        Src[(iz * nx * ny) + l * nx + k] * weight1 * weight2 * intersection_length[
                                            pix_num])
            ...
            if k >= 0 and k <= nx - 1:
                ...
                for l in range(s_index_z, e_index_z + 1):
                    if l >= 0 and l <= nz - 1:
                        weight2 = weight_calculator1(l, s_index_z, e_index_z, coord_z1, coord_z2, dz, Zplane[l],
                                                     Zplane[l + 1])
                        cuda.atomic.add(Dest, pix_num, Src[(l * nx * ny) + iy * nx + k] * weight1 * weight2)
            ...
            if k >= 0 and k <= ny - 1:
                ...
                for l in range(s_index_z, e_index_z + 1):
                    if l >= 0 and l <= nz - 1:
                        weight2 = weight_calculator1(l, s_index_z, e_index_z, coord_z1, coord_z2, dz, Zplane[l],
                                                     Zplane[l + 1])
                        cuda.atomic.add(Dest, pix_num, Src[(l * nx * ny) + k * nx + ix] * weight1 * weight2)
...
def flat_distance_backproj_arb(Dest, Src, Xplane, Yplane, Zplane, Uplane, Vplane, param):
    ...
    if tid < N:
        ...
        yc = fy(Xplane[ix], Yplane[iy], Zplane[iz], angle1, angle2)

        x1 = fx(Xplane[ix] + dx / 2.0, Yplane[iy] + dy / 2.0, Zplane[iz] - dz / 2.0, angle1, angle2)
        y1 = fy(Xplane[ix] + dx / 2.0, Yplane[iy] + dy / 2.0, Zplane[iz] - dz / 2.0, angle1, angle2)
        z1 = fz(Xplane[ix] + dx / 2.0, Yplane[iy] + dy / 2.0, Zplane[iz] - dz / 2.0, angle1, angle2)

        x2 = fx(Xplane[ix] - dx / 2.0, Yplane[iy] - dy / 2.0, Zplane[iz] - dz / 2.0, angle1, angle2)
        y2 = fy(Xplane[ix] - dx / 2.0, Yplane[iy] - dy / 2.0, Zplane[iz] - dz / 2.0, angle1, angle2)
        z2 = fz(Xplane[ix] - dx / 2.0, Yplane[iy] - dy / 2.0, Zplane[iz] - dz / 2.0, angle1, angle2)

        x3 = fx(Xplane[ix] + dx / 2.0, Yplane[iy] - dy / 2.0, Zplane[iz] - dz / 2.0, angle1, angle2)
        y3 = fy(Xplane[ix] + dx / 2.0, Yplane[iy] - dy / 2.0, Zplane[iz] - dz / 2.0, angle1, angle2)
        z3 = fz(Xplane[ix] + dx / 2.0, Yplane[iy] - dy / 2.0, Zplane[iz] - dz / 2.0, angle1, angle2)

        x4 = fx(Xplane[ix] - dx / 2.0, Yplane[iy] + dy / 2.0, Zplane[iz] - dz / 2.0, angle1, angle2)
        y4 = fy(Xplane[ix] - dx / 2.0, Yplane[iy] + dy / 2.0, Zplane[iz] - dz / 2.0, angle1, angle2)
        z4 = fz(Xplane[ix] - dx / 2.0, Yplane[iy] + dy / 2.0, Zplane[iz] - dz / 2.0, angle1, angle2)

        x5 = fx(Xplane[ix] + dx / 2.0, Yplane[iy] + dy / 2.0, Zplane[iz] + dz / 2.0, angle1, angle2)
        y5 = fy(Xplane[ix] + dx / 2.0, Yplane[iy] + dy / 2.0, Zplane[iz] + dz / 2.0, angle1, angle2)
        z5 = fz(Xplane[ix] + dx / 2.0, Yplane[iy] + dy / 2.0, Zplane[iz] + dz / 2.0, angle1, angle2)

        x6 = fx(Xplane[ix] - dx / 2.0, Yplane[iy] - dy / 2.0, Zplane[iz] + dz / 2.0, angle1, angle2)
        y6 = fy(Xplane[ix] - dx / 2.0, Yplane[iy] - dy / 2.0, Zplane[iz] + dz / 2.0, angle1, angle2)
        z6 = fz(Xplane[ix] - dx / 2.0, Yplane[iy] - dy / 2.0, Zplane[iz] + dz / 2.0, angle1, angle2)

        x7 = fx(Xplane[ix] + dx / 2.0, Yplane[iy] - dy / 2.0, Zplane[iz] + dz / 2.0, angle1, angle2)
        y7 = fy(Xplane[ix] + dx / 2.0, Yplane[iy] - dy / 2.0, Zplane[iz] + dz / 2.0, angle1, angle2)
        z7 = fz(Xplane[ix] + dx / 2.0, Yplane[iy] - dy / 2.0, Zplane[iz] + dz / 2.0, angle1, angle2)

        x8 = fx(Xplane[ix] - dx / 2.0, Yplane[iy] + dy / 2.0, Zplane[iz] + dz / 2.0, angle1, angle2)
        y8 = fy(Xplane[ix] - dx / 2.0, Yplane[iy] + dy / 2.0, Zplane[iz] + dz / 2.0, angle1, angle2)
        z8 = fz(Xplane[ix] - dx / 2.0, Yplane[iy] + dy / 2.0, Zplane[iz] + dz / 2.0, angle1, angle2)
