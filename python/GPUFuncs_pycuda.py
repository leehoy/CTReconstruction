...
def DefineGPUFuns():
    ...
    __global__ void distance_project_on_y3(float* Dest,float* Src,float* slope_x1,float* slope_x2,float* slope_z1,float* slope_z2,float* intercept_x1,float* intercept_x2,float* intercept_z1,float* intercept_z2,float* Xplane,float* Yplane,float* Zplane,float* param){
            int x=blockDim.x*blockIdx.x+threadIdx.x;
            int y=blockDim.y*blockIdx.y+threadIdx.y;
            int tid=y*gridDim.x*blockDim.x+x;
            float dx=param[0];
                    ...
                    if(k>=0 && k<= nx-1){
                        ...
                        for(l=s_index_z;l<=e_index_z;l++){
                            if(l>=0 && l<= nz-1){
                                ...
                                atomicAdd(&Dest[pix_num+angle_ind*N],Src[(l*nx*ny)+iy*nx+k]*weight1*weight2);
                            }
    ...
    __global__ void distance_project_on_x3(float* Dest,float* Src,float* slope_y1,float* slope_y2,float* slope_z1,float* slope_z2,float* intercept_y1,float* intercept_y2,float* intercept_z1,float* intercept_z2,float* Xplane,float* Yplane,float* Zplane,float* param){
            int x=blockDim.x*blockIdx.x+threadIdx.x;
            int y=blockDim.y*blockIdx.y+threadIdx.y;
            int tid=y*gridDim.x*blockDim.x+x;
            float dx=param[0];
                    ...
                    if(k>=0 && k<= ny-1){
                        ...
                        for(l=s_index_z;l<=e_index_z;l++){
                            if(l>=0 && l<=nz-1){
                                ...
                                atomicAdd(&Dest[pix_num+angle_ind*N],Src[(l*nx*ny)+k*nx+ix]*weight1*weight2);
                            }
                        }
                    }
                }
    ...
    __global__ void distance_project_on_z3(float* Dest,float* Src,float* slope_x1,float* slope_x2,float* slope_y1,float* slope_y2,float* intercept_x1,float* intercept_x2,float* intercept_y1,float* intercept_y2,float* Xplane,float* Yplane,float* Zplane,float* param){
            int x=blockDim.x*blockIdx.x+threadIdx.x;
            int y=blockDim.y*blockIdx.y+threadIdx.y;
            int tid=y*gridDim.x*blockDim.x+x;
            float dx=param[0];
                    ...
                    if(k>=0 && k<= nx-1){
                        ...
                        for(l=s_index_y;l<=e_index_y;l++){
                            if(l>=0 && l<=ny-1){
                                ...
                                atomicAdd(&Dest[pix_num+angle_ind*N],Src[(iz*nx*ny)+l*nx+k]*weight1*weight2);
                            }
                        }
                    }
                }
